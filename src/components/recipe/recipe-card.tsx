...
export function RecipeCard({ recipe, href }: { recipe: any; href?: string }) {
    ...
    const candidates = useMemo(() => {
        ...
        // Check static catalog match by name, slug, or ID
        const matched = allRecipes.find(r => 
            r.id === recipe?.id || 
            r.id === recipe?.recipeId || 
            r.id === recipe?.originalId ||
        ...
        if (cleanName) {
            ...
            if (slug) list.push(...getRecipeImageCandidates(slug));
        }
                    ...
                    <Card className="flex flex-col h-full rounded-[2rem] bg-card/90 backdrop-blur-md border border-stone-200/80 dark:border-stone-800/80 shadow-sm hover:shadow-xl transition-all duration-300 group overflow-hidden">
                        ...
                        <div className="relative h-44 w-full bg-stone-100 dark:bg-stone-900 overflow-hidden shrink-0">
                            ...
                            <div className="absolute top-3 left-3 right-3 flex items-center justify-between gap-1.5 pointer-events-none">
                                <Badge variant={displayType === 'Vegetarian' ? 'secondary' : 'destructive'} className="font-semibold text-[10px] uppercase tracking-wider px-2.5 py-0.5 rounded-full shadow-sm">
                                    {displayType}
                                </Badge>
                                {recipe?.menuCategory && (
                            ...
                            <div className="absolute bottom-2.5 left-3 right-3 flex items-center gap-2 text-white text-[11px] font-medium drop-shadow-md">
                                <span className="flex items-center gap-1 bg-black/40 backdrop-blur-md px-2 py-0.5 rounded-full border border-white/10">
                                    <Clock className="h-3 w-3 text-amber-400" /> {displayTime}m
                                </span>
                                <span className="flex items-center gap-1 bg-black/40 backdrop-blur-md px-2 py-0.5 rounded-full border border-white/10">
                                    <RupeeIcon className="h-3 w-3 text-emerald-400" /> ₹{displayCost}
                                </span>
                                <span className="flex items-center gap-1 bg-black/40 backdrop-blur-md px-2 py-0.5 rounded-full border border-white/10 ml-auto">
                                    <Users className="h-3 w-3 text-sky-400" /> {displayServings}
                                </span>
                            </div>
                        </div>
                            ...
                            <div className="border-t border-stone-200 dark:border-stone-800 pt-3">
                                ...
                                <div className="flex flex-wrap gap-1.5">
                                    {(recipe?.ingredients || []).slice(0, 4).map((ing: any, idx: number) => {
                                        ...
                                        return (
                                            <Badge key={ingKey} variant="outline" className="text-[11px] font-medium bg-stone-100 dark:bg-stone-800/80 border-stone-200 dark:border-stone-700 py-1 px-2.5 rounded-lg">
                                                {ingName}
                                            </Badge>
                                        );
                                    })}
